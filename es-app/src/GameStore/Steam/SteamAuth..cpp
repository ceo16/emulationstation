...
bool SteamAuth::validateAndSetAuthentication()
{
        ...
        try {
                ...
                !responseJson["response"]["players"].empty())
            {
                ...
                if (!mUserPersonaName.empty() && steamIdFromJson == mSteamId) {
                    mIsAuthenticated = true; // Imposta il flag
                    // AGGIUNGI QUESTO LOG SUBITO DOPO:
                    LOG(LogDebug) << "SteamAuth: Flag mIsAuthenticated impostato a true. Valore attuale: " << mIsAuthenticated;
	...
	LOG(LogDebug) << "SteamAuth: validateAndSetAuthentication termina. Valore mIsAuthenticated: " << mIsAuthenticated;
