...
void HttpServerThread::run() {
    ...
    mHttpServer->Get("/quit", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        if (req.has_param("confirm") && req.get_param_value("confirm") == "switchscreen") {
            Window* win = mWindow;
            mWindow->postToUiThread([win]() {
                win->pushGui(new GuiMsgBox(win, _("DO YOU WANT TO SWITCH THE SCREEN ?"), _("YES"), [] { Utils::Platform::quitES(); }, _("NO"), nullptr));
            });
            return;
        }
#endif
                ...
                if (game->getMetadata().getType(metadataName) == MD_PATH) {
                    if (HttpApi::ImportMedia(game, metadataName, contentType, req.body)) {
                        if (ViewController::hasInstance())
                            mWindow->postToUiThread([game]() {
                            ViewController::get()->onFileChanged(game, FileChangeType::FILE_METADATA_CHANGED);
                        });
    ...
    mHttpServer->Post("/messagebox", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        auto msg = req.body;
        Window* w = mWindow;
        mWindow->postToUiThread([msg, w]() {
            w->pushGui(new GuiMsgBox(w, msg));
        });
    });
    ...
    mHttpServer->Post(R"(/addgames/(/?.*))", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        if (deleteSystem) {
            ...
            mWindow->postToUiThread([system]() {
                ViewController::get()->onFileChanged(system->getRootFolder(), FILE_METADATA_CHANGED);
            });
        }
    ...
    // Task queue processing
    {
        ...
        if (mTaskQueue.empty()) {
            mQueueCondition.wait_for(lock, std::chrono::milliseconds(100));
        }
        if (!mTaskQueue.empty()) {
