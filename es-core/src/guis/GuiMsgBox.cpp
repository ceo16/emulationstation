...
GuiMsgBox::GuiMsgBox(Window* window, const std::string& text, const std::string& title,
	const std::string& name1, const std::function<void()>& func1,
	const std::string& name2, const std::function<void()>& func2,
	const std::string& name3, const std::function<void()>& func3,
	const std::string& name4, const std::function<void()>& func4,
	GuiMsgBoxIcon icon) : GuiComponent(window),
	mBackground(window, ":/frame.png"), mGrid(window, Vector2i(2, 2))  // Or appropriate grid size
{
	auto theme = ThemeData::getMenuTheme();
	...
	mMsg = std::make_shared<TextComponent>(mWindow, text, ThemeData::getMenuTheme()->Text.font, ThemeData::getMenuTheme()->Text.color, mImage == nullptr || Renderer::isSmallScreen() ? ALIGN_CENTER : ALIGN_LEFT);
	mMsg->setPadding(Vector4f(Renderer::getScreenWidth() * 0.015f, 0, Renderer::getScreenWidth() * 0.015f, 0));
	...
	if (!name3.empty())
		mButtons.push_back(std::make_shared<ButtonComponent>(mWindow, name3, name3, std::bind(&GuiMsgBox::deleteMeAndCall, this, func3)));
	if (!name4.empty())
		mButtons.push_back(std::make_shared<ButtonComponent>(mWindow, name4, name4, std::bind(&GuiMsgBox::deleteMeAndCall, this, func4)));
